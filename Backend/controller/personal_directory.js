...
module.exports.renamePosition = async (req,res) => {
    ...
    try {
        db.query(`SELECT * FROM person_directory_position WHERE pd_position_name = ? AND pd_category_id = ?` ,
        [position_name,position_category_id],async (err,result)=>{ 
            if(err )return return_err(res,'QEURY BLOCK',' CHECK NAME',err,500,'SERVER ERROR')
            if(result.length >= 1){
                res.status(200).json({
                    status_code:409,
                    status: false ,
                ...
                try{
                    ...
                    `,[position_name , position_id], async (err,result)=>{
                        if(err) return return_err(res,'QEURY BLOCK 2','RENAME POSITION',err,500,'SERVER ERROR')
                    ...
                    return return_err(res,'TRY CATCH BLOCK 2 ','UPDATE POSITION',err,500,'SERVER ERROR')
                }
            }
        })
    }catch{
        return return_err(res,'TRY CATCH BLOCK','UPDATE POSITION',err,500,'SERVER ERROR')
...
module.exports.addPerson = async (req,res)=>{
    ...
    const person_phone = req.body.person_phone
    ...
    // try{
    //     db.query(`
    //             INSERT INTO person_directory_persons(
    //                 pd_person_image,
    //                 pd_person_name,
    //                 pd_person_descript,
    //                 pd_position_id,
