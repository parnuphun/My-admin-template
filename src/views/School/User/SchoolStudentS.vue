...
import { StudentSResponse , classListResponse} from '../../../store/Interface'
...
const crrImg = ref()
const teachingSList = ref<Array<StudentSResponse>>()

 
function getTeachingSClient(){
    _api.getStudentSClient({class_id:classSelected.value}).then((res)=>{
        if(res.data.status_code === 200){
            teachingSList.value = res.data.data_list
            baseImagePaht.value = res.data.base_image_path   
            tableSelected.value = teachingSList.value![0].ss_id
            crrImg.value = teachingSList.value![0].ss_img
        }
    })
}
...
watch(tableSelected,()=>{
    let index = teachingSList.value!.findIndex(obj => obj.ss_id === tableSelected.value);
    crrImg.value = teachingSList.value![index].ss_img
})
